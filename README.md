...
Extra: If you need to rename script/log/checkpoint filenames uniformly (full replacement version)
If you want all file/log/checkpoint names to match FusioBERT (no residual RPB/PepRPB), here is the fully polished version:
-FusioBERT
...
│
├── models/                 # Model architecture
│   └── fusiobert_model.py  # FusioBERT model definition
│
├── train/                  # Training scripts
│   ├── train_fusiobert.py  # Main pre-training script
│   
│
├── inference/              # eval scripts
│   └── fusiobert_inference.py  # Predict candidate peptides
│
└── data/                   # Example data (no full dataset)
    ├── example_candidates.csv  # Example candidate peptides (10-20 samples)
    └── amp_classification.csv  # Data for training classification head
🏋️ Model Training
1. Pre-training
Run the pre-training script for FusioBERT:
bash
运行
cd FusioBERT
nohup python -u train/train_fusiobert.py > logs_fusiobert.log 2>&1 
Key pre-training details:
Pre-training data: Local parquet file (processed from UniParc)
Sampling: 19.18 million sequences per epoch (aligned with PepBERT-large)
Learning rate scheduling: SGDR + Warmup
Checkpoints: Saved to checkpoints_fusiobert/
Logs: Saved to logs_fusiobert.log
2. Fine-tuning (Classification Head)
Fine-tune the pre-trained model with a classification head for AMP identification (script not included here; use the classification head defined in inference/fusiobert_inference.py).
🔮 Inference for Candidate Peptides
Use the pre-trained FusioBERT model to predict AMP labels for candidate peptides (e.g., candidates_physicochem_filtered.csv).
Step 1: Prepare Input Data
Prepare candidate peptides in CSV format (example: data/example_candidates.csv):
...
bash
运行
python inference/fusiobert_inference.py
Step 3: View Results
Prediction results are saved to data/fusiobert_amp_prediction.csv, including:
sequence: Candidate peptide sequence
predicted_label: 1 (AMP) / 0 (non-AMP)
amp_probability: Probability of being an AMP (0-1)
